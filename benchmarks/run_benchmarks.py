...
# Load .env from project root (/NLP-PDC-project/.env)
...
def build_env_for_case(args: argparse.Namespace, case: BenchmarkCase) -> dict[str, str]:
    ...
    # Only override if explicitly provided — otherwise .env handles it

    if args.mock:
        env["USE_MOCK_ENGINE"] = "true"

    if case.fast_path_mode:
        env["FAST_PATH_MODE"] = case.fast_path_mode

    if args.fast_model:
        env["LLAMA_FAST_MODEL_DIR"] = args.fast_model

    if args.qwen_model:
        env["QWEN_AUTOCOMPLETE_MODEL_DIR"] = args.qwen_model

    if args.granite_fim_model:
        env["GRANITE_FIM_MODEL_DIR"] = args.granite_fim_model

    if args.target_model:
        env["LLAMA_REWRITE_TARGET_MODEL_DIR"] = args.target_model

    if case.draft_model_path:
        env["LLAMA_REWRITE_DRAFT_MODEL_DIR"] = case.draft_model_path

    # Generation params (these SHOULD override .env per experiment)
    env.update({
